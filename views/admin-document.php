...
$document = (new Document())->getAdminDocument((int) $documentId);
...
<html lang="en">
  ...
  <body class="admin-body">
    ...
    <main class="admin-page admin-document-view-page">
      ...
      <section class="admin-table-panel" aria-label="Document audit trail">
        <div class="admin-document-section-heading"><div><h2>Audit Trail</h2><p>Permanent document actions recorded separately from the planned route.</p></div></div>
        <div class="admin-table-scroll">
          <table id="adminDocumentAuditTable" class="display admin-data-table">
            <thead><tr><th>Date</th><th>Action</th><th>Action By</th><th>Office Movement</th><th>Remarks</th></tr></thead>
            <tbody>
              <?php foreach ($auditTrail as $entry): ?>
                <tr>
                  ...
                  <td><span class="admin-status-pill"><?= htmlspecialchars((string) $entry['action'], ENT_QUOTES, 'UTF-8') ?></span></td>
    ...
    <script>
      ...
      new DataTable('#adminDocumentAuditTable', {
        paging: false,
        searching: false,
        info: false,
        order: [[0, 'asc']],
        language: { emptyTable: 'No audit events have been recorded.' }
      });
    </script>
  </body>
</html>
