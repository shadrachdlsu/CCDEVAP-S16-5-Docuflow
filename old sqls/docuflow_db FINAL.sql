...
(1, 1, NULL, 'System Administrator', 'admin@docuflow.local', '$2y$10$nnErFsaStXuxeMvz4p9f0ux4gBjjr.BFNUgk97xNlItMP8R9MA5dy', 1, 'Approved', '2026-06-01 00:00:00'),
(2, 2, 1, 'Rosa Registrar', 'registrar@docuflow.local', '$2y$10$cxLnE59H1/vvWjAWxENgjuKjXMu90IDp6new/9OegwJsQ4j4xHUBC', 1, 'Approved', '2026-06-01 00:05:00'),
(3, 2, 2, 'Felix Finance', 'finance@docuflow.local', '$2y$10$8gY1LZAe8XZTpLJL3vVkw.UTLG5QmUMFW5lOySGW2HT9P69Cudb/C', 1, 'Approved', '2026-06-01 00:10:00'),
(4, 2, 3, 'Diana Dean', 'dean@docuflow.local', '$2y$10$C/faAPl4eGtVGwvYjtP38.nNIQeyaX5.qRonTNUj1E/eC4PdN1jSu', 1, 'Approved', '2026-06-01 00:15:00'),
(5, 3, 1, 'Marco Member', 'member@docuflow.local', '$2y$10$YMgLXkYwWt2HcMaCsVDK1.Z2X1aQWdLJEt4IuKfJwUooYWFbdLHHC', 1, 'Approved', '2026-06-01 00:20:00');
...
--
ALTER TABLE `document_type_offices`
  ADD CONSTRAINT `fk_document_type_offices_type` FOREIGN KEY (`type_id`) REFERENCES `document_types` (`type_id`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `fk_document_type_offices_office` FOREIGN KEY (`office_id`) REFERENCES `offices` (`office_id`) ON DELETE CASCADE ON UPDATE CASCADE;
